...
mod tests {
  ...
  #[test]
  fn planned_indexes_orders_dependencies_for_start_and_reverses_for_end() {
    let mut store = OrchestratorStore::default();
    push_collect_start(&mut store, "a", &[]);
    push_collect_start(&mut store, "b", &["a"]);
    push_collect_start(&mut store, "c", &["b"]);

    let start = store
      .planned_indexes(BootCycle::Collect, BootPhase::Start)
      .expect("start planning should succeed");
    let start_ids: Vec<&str> = start.iter().map(|idx| store.list[*idx].id()).collect();
    assert_eq!(start_ids, vec!["a", "b", "c"]);
    ...
    let end = store
      .planned_indexes(BootCycle::Collect, BootPhase::End)
      .expect("end planning should succeed");
    let end_ids: Vec<&str> = end.iter().map(|idx| store.list[*idx].id()).collect();
    assert_eq!(end_ids, vec!["end-b", "end-a"]);
  }

  #[test]
  fn planned_indexes_detects_dependency_cycles() {
    let mut store = OrchestratorStore::default();
    push_collect_start(&mut store, "a", &["b"]);
    push_collect_start(&mut store, "b", &["a"]);
