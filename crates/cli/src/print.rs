...
pub fn print_units(units: &[UnitSerialized]) {
  ...
  for u in units {
    println!(
      ...
      format!("{}/{} | {}", u.active_facets, u.facets, u.impulses).bright_purple(),
    );
  }
}
...
pub fn print_unit(unit_name: &String, unit: &UnitItemsSerialized) {
  ...
  if !unit.facets.is_empty() {
    println!("{}", " Facets ".on_bright_green().bold().white());
    for s in &unit.facets {
      println!(
        "  {:<20} {:<5} {:<}",
        s.name.to_string().bold().white(),
  ...
  if !unit.impulses.is_empty() {
    println!("{}", " Impulses ".on_bright_green().bold().white());
    for s in &unit.impulses {
...
pub fn print_state(st: &FacetSerialized) {
