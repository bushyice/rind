...
fn inject_networking(name: &str, mut metadata: Metadata) -> CoreResult<Metadata> {
  match name {
...
struct NetworkingOrchestrator;

impl Orchestrator for NetworkingOrchestrator {
  fn id(&self) -> &str {
    "networking"
  }
  ...
  fn runtimes(&self) -> Vec<Box<dyn Runtime>> {
    vec![Box::new(NetworkingRuntime::default())]
  }
}

struct NetworkingPumpOrchestrator;

impl Orchestrator for NetworkingPumpOrchestrator {
  fn id(&self) -> &str {
    "networking-pump"
  }
